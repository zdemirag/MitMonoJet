...
grep -v ^# ${MIT_MONOJET_DIR}/config/${MIT_PROD_CFG}.txt > /tmp/processMonoJet.$$
nLine=0

for dset in `cat /tmp/processMonoJet.$$|grep -v ^#|tr -s ' '|cut -d' ' -f 2`
do

  nLine=$(($nLine+1))

  # find the line to this dataset and do further analysis
  line=`sed -n ${nLine}p /tmp/processMonoJet.$$`
  ...
  #echo $MIT_PROD_MACRO $MIT_CATALOG   $BOOK_VERSION   $DATASET $SKIM $MIT_PROD_CFG   $MIT_PROD_HIST $CHECK_COMPLETE
done

rm /tmp/processMonoJet.$$
